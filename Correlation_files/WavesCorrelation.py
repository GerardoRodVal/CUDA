...
# ------------------------ scale by sqrt(norm(s1)*norm(s2win)) where s2win is the moving window of s2 ------------------
    ...
    scal = array( np.sqrt(scal) ) * array( np.linalg.norm(s1) )
...
cont = 0
for i in glob.glob('*.sac'):
    for j in glob.glob('*.sac'):
        if( i != j ):
            pass
            ...
            s1 = sac1f[0].data[:50500]                                                                                 # solamento el vector de ondas
            s2 = sac2f[0].data
            ...
            plt.savefig( 'correlation_'+str(cont)+'.png' )
            cont += 1
