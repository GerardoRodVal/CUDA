...
for i in range(batch-1):								# 45 comparaciones,  num. comparaciones definidas por -->  ((bath)*(bath-1))/2 
	s1 = Vector( V1, inicio, final)						# creando vector individual a partir del vector de final de fft
	inicio2 = MAX + pasos
	final2 = (MAX+MAX) + pasos

	for i in range(batch-n):
		s2 = Vector( V2, inicio2, final2 )				# creando el vector individual apartir del vector dinal de complejo conjugado
